...
from my_module import get_emoji, load_model
...
base_output = get_emoji("", model, tokenizer)


@app.get("/{input_text}")
def read_root(input_text: str):
    output = get_emoji(demojize(input_text), model, tokenizer)
    lift = (output.sort_index() / base_output.sort_index()).sort_values(ascending=False)
    sorted_emojis = emojize("".join(lift.index))

    # return {"input": input_text, "prediction": sorted_emojis}
    return {
        "emoji": sorted_emojis,
        "abilities": [
            {
